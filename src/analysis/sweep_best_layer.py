...
def main() -> None:
    ...
    print(f"{'Layer':>6}  {'r_value':>9}  {'p_value':>10}  {'rho_time':>9}  {'p_time':>10}")
    print("-" * 65)
    ...
    for i in range(n_layers):
        ...
        print(f"  L{i+1:02d}  {r_val:>+9.4f}  {p_val:>10.3e}  {rho_t:>+9.4f}  {p_t:>10.3e}")

    print("=" * 65)

    best = max(results, key=lambda x: x["r_value"])
    print(f"\n  Best value-axis layer: L{best['layer']:02d}  "
          f"r={best['r_value']:+.4f}  rho_time={best['rho_time']:+.4f}")
